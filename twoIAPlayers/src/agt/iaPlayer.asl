...
// =======================
// RECEIVE MOVE — el oponente jugó, actualizamos y respondemos
// solo si la partida no ha terminado ya
// =======================
+played(Simb,X,Y) : otherSymbol(Simb) & realBoardState(Board) & not gameOver <-
    ...
    // Comprobamos si el oponente ganó con su jugada
...
+!playMove(X,Y) : mySymbol(Simb) & realBoardState(Board) <-
    ...
    // Comprobamos si ganamos con esta jugada
    if (winState(NewBoard, Simb)) {
        +gameOver;
        .my_name(Me);
...
// =======================
// RECEIVE GAME RESULT (enviado por el otro agente)
// =======================
